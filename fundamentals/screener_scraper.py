"""Data ingestion helpers covering Screener.in scraping and ticker discovery."""
    ...
    *,
    ...
    try:
        candidates = [
            ...
            os.path.join("source", "ind_nifty500list.csv"),
        ]
        script_dir = os.path.dirname(os.path.abspath(__file__))
        candidates.append(os.path.join(script_dir, "source", "ind_nifty500list.csv"))
        candidates.append(os.path.join(script_dir, "ind_nifty500list.csv"))
    ...
    raise RuntimeError("Failed to download Nifty 500 list") from last_error


class ScreenerScraper:
    base_url = "https://www.screener.in/company/{ticker}/"
    # Repo stores these lists under `source/`.
    ticker_map_file = "source/ticker_mapping.json"
    consolidated_file = "source/consolidated.json"
    non_consolidated_file = "source/nonconsolidated.json"
    ...
    def __init__(
        ...
        *,
        ...
        # Load strict URL lists with robust path handling
    ...
    def _load_ticker_map(self) -> Dict[str, str]:
        ...
        path = self._resolve_path(self.ticker_map_file)
    ...
    def _load_list(self, filename: str) -> Set[str]:
        ...
        if path and os.path.exists(path):
            try:
                ...
                # Silent fail for now or print warning only once
    ...
    def _resolve_path(self, filename: str) -> Optional[str]:
        ...
        if os.path.exists(filename):
            return filename
        # Common repo layout: files stored under ./source/
        ...
        if os.path.exists(path):
            return path
        path = os.path.join(script_dir, "source", filename)
        if os.path.exists(path):
            return path
        # Check in parent dir (if running from src)
        path = os.path.join(os.path.dirname(script_dir), filename)
        if os.path.exists(path):
            return path
        path = os.path.join(os.path.dirname(script_dir), "source", filename)
        if os.path.exists(path):
            return path
        return None
    ...
    def _save_ticker_map(self) -> None:
        ...
        try:
            path = self._resolve_path(self.ticker_map_file) or self.ticker_map_file
    ...
    def _fetch_strictly(self, ticker: str) -> Tuple[str, str]:
        ...
        - If in neither -> Default to Consolidated (as per Nifty 500 norm).
        """
