...
# Allow importing modules from the `fundamentals/` folder when running this file directly.
...
def _load_master_industry_map() -> list[dict]:
...
def _build_ticker_to_industry_and_pe(master_map: list[dict]) -> tuple[dict[str, str], dict[str, float]]:
...
def process_ticker(
    ...
    *,
    ...
    if not force:
        try:
            ...
            if existing and not SupabaseManager.needs_refresh(existing):
                logging.info("Processed %s: skipping (data is fresh)", ticker)
                return
    ...
    for attempt in range(max_retries):
        try:
            # Scraper internally enforces Consolidated vs Standalone via JSON lists.
            # No auto-switching occurs. 404s or unexpected redirects raise errors.
            ...
            if "429" in error_msg:
                ...
                # Strict mode: 404 means data is missing at the expected URL.
                logging.error("Processed %s: failed (404 Not Found at expected URL)", ticker)
                return # Don't retry 404s
            
            elif "Strict Check Failed" in error_msg:
                 # Custom error from scraper when URL doesn't match expectation
                 logging.error("Processed %s: failed (%s)", ticker, error_msg)
                 return
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        manager = SupabaseManager()
        scraper = ScreenerScraper(use_industry_pe_map=False) # industry PE handled once below
    except Exception as e:
        logging.critical("Failed to initialize backend services: %s", e)
        return 1

    # 0. Map Nifty 500 -> industry and load Industry PE from run_industry_check output.
    master_map = _load_master_industry_map()
    ticker_to_industry, industry_pe_map = _build_ticker_to_industry_and_pe(master_map)
    if not master_map:
        logging.warning(
            "master_industry_map.json missing. Run fundamentals/source/run_industry_check.py ALL to generate it."
        )
