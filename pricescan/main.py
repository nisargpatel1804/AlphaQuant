...
from pricescan.config import (
    PRICESCAN_RESULTS_DIR, 
    NIFTY_500_CSV_PATH,
    BENCHMARK_TICKER
)
from pricescan.fetcher import PriceScanFetcher
from pricescan.scans import PriceScanner
from pricescan.sector_manager import SectorManager
...
class PriceScanEngine:
    ...
    def process_ticker(self, ticker: str) -> Optional[TickerPriceScanData]:
        ...
        triggered_total = len(scan_results)
        
        # 6. Group Results
        # Grouping by 'status' (Bullish/Bearish) for the UI
        grouped_results = {
            "Bullish": [],
            "Bearish": [],
            "Neutral": [],
        ...
        for res in scan_results:
            if res.condition_met:
                ...
                # If status is "Bullish", it goes to Bullish bucket, etc.
                # If status is internal logic text, we might need mapping, 
                # but PriceScanner.run_all_scans currently sets status to "Bullish"/"Bearish" directly.
                bucket = res.status
                if bucket not in grouped_results:
                    bucket = "Neutral" # Fallback
                
                grouped_results[bucket].append(res.to_dict())

            grouped_triggered_total = sum(len(v) for v in grouped_results.values())
        ...
        output_data = TickerPriceScanData(
            ...
            scan_results=grouped_results,
            scan_summary={
                "expected_total": coverage.get("expected", 0),
                "implemented_total": coverage.get("implemented", 0),
                "triggered_total": grouped_triggered_total,
                "triggered_total_raw": triggered_total,
                "by_bucket": {k: len(v) for k, v in grouped_results.items()},
            },
        )
    ...
    def save_results(self, data: TickerPriceScanData):
        ...
        try:
            with open(file_path, "w", encoding="utf-8") as f:
                # Convert dataclass to dict, handling the nested dict structure manually
                # or using simple dict construction since TickerPriceScanData is simple
                json_dict = {
                    "ticker": data.ticker,
...
def main():
    ...
    if args.ticker:
        ...
        # SectorManager loads the map on init
        # We can scan the ticker_to_industry map
    ...
    for i, ticker in enumerate(target_tickers):
        try:
            ...
            if result:
                ...
                logger.info(f"[{i+1}/{len(target_tickers)}] {ticker}: Bullish={bull}, Bearish={bear}")
