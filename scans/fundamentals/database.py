...
class SupabaseManager:
    ...
    def __init__(
        ...
        *,
        ...
        if not self.url or not self.key:
            # We raise a ValueError here, but in production this might just log a warning
            # and disable DB features if the app can run stateless.
            # For now, strict validation.
            # raise ValueError("Supabase credentials are missing. Set SUPABASE_URL and a key env var.")
            # Graceful degradation for dev environment:
            self.client = None
            return
    ...
    def execute_sql(self, statement: str) -> None:
        ...
        """
        if not statement.strip() or not self.client:
            return
        ...
        try:
            ...
            pass # Swallow RPC errors in dev if function doesn't exist
    ...
    def upsert_record(self, payload: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Log error but don't crash app if DB is down
