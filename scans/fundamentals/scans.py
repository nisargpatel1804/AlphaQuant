...
def _build_scan_definitions() -> Tuple[ScanDefinition, ...]:
    ...
    efficiency = [
        ...
        ("check_fixed_asset_turnover", "Fixed Asset Efficiency"),
    ]
...
class FundamentalScans:
    """
    Evaluates fundamental metrics and returns categorized results with signals.
    """
    ...
    def _map_status_to_action(status_text: str) -> str:
        """Map fundamental status text to a consistent 5-level action signal."""
        t = (status_text or "").strip().lower()
        
        # Strong Buy indicators (High Quality / Undervalued)
        if "undervalued" in t or "debt free" in t or "consistent growth" in t or "clean" in t:
            return "Strong Buy"
        
        # Buy indicators (Good / Positive)
        if "high" in t and "debt" not in t and "risk" not in t and "expensive" not in t:
            return "Buy"
        if "positive" in t or "improving" in t or "cheap" in t or "accumulating" in t or "strong" in t:
            return "Buy"
        if "low debt" in t or "low pledge" in t:
            return "Buy"

        # Strong Sell indicators (High Risk / Expensive)
        if "expensive" in t or "high pledge" in t or "worsening" in t:
            return "Strong Sell"
            
        # Sell indicators (Weak / Negative)
        if "low" in t and "debt" not in t and "pledge" not in t and "pe" not in t:
            return "Sell" # Low growth, low margin, etc.
        if "negative" in t or "tight" in t or "distributing" in t or "inconsistent" in t:
            return "Sell"
        if "high debt" in t or "premium" in t: # Premium valuation is often a sell/trim signal in value investing
            return "Sell"

        # Neutral indicators
        if "moderate" in t or "fair" in t or "stable" in t or "adequate" in t or "reasonable" in t or "flat" in t:
            return "Neutral"
    ...
    def _calculate_category_signal(self, scans: List[Dict[str, Any]]) -> str:
        ...
        # Thresholds for category-level signal
        if avg_score >= 1.2: return "Strong Buy" # High quality
        if avg_score >= 0.3: return "Buy"        # Good quality
        if avg_score <= -1.2: return "Strong Sell" # Poor quality
        if avg_score <= -0.3: return "Sell"        # Weak quality
        return "Neutral"
    ...
    def check_net_profit_growth_qtr(self) -> str:
        ...
        if len(periods) <= 4: return "Pending"
        curr = self._get_value(self.quarterly, 0, "net_profit")
        prev = self._get_value(self.quarterly, 4, "net_profit")
    ...
    def check_earnings_consistency(self) -> str:
        ...
        if len(eps_series) < 3 or any(e is None for e in eps_series): return "Pending"
        self._record_value(eps_series[0])
        if eps_series[0] > eps_series[1] > eps_series[2]: return "Consistent Growth"
        if eps_series[0] > eps_series[1]: return "Growth"
        return "Inconsistent"
    ...
    def check_ev_ebitda_trend(self) -> str:
        ...
        if market_cap is None: return "Pending"
        ...
        if len(op) < NUMBER_OF_QUARTERS_IN_YEAR or len(dep) < NUMBER_OF_QUARTERS_IN_YEAR: return "Pending"
        if any(v is None for v in op) or any(v is None for v in dep): return "Pending"
        ebitda_ttm = float(sum((float(o) + float(d)) for o, d in zip(op, dep)))
        if ebitda_ttm <= 0: return "Pending"
        ...
        debt_f = float(borrowings or 0)
        ev = float(market_cap) + debt_f - cash_f
        ev_ebitda = self._safe_divide(ev, ebitda_ttm)
        if ev_ebitda is None: return "Pending"
        ev_ebitda = float(ev_ebitda)
        self._record_value(ev_ebitda)
        if ev_ebitda < 8: return f"Undervalued (EV/EBITDA {ev_ebitda:.1f}x)"
        if ev_ebitda < 12: return f"Fair Value (EV/EBITDA {ev_ebitda:.1f}x)"
        if ev_ebitda < 18: return f"Premium Valuation (EV/EBITDA {ev_ebitda:.1f}x)"
    ...
    # Public Execution
    # ------------------------------------------------------------------
    def run_scans(self) -> Dict[str, Dict[str, Any]]:
        """
        Executes all defined checks. 
        Returns a dictionary grouped by CATEGORY (Profitability, Solvency, etc.)
        containing the aggregate signal and list of individual scans.
        """
