...
def resample_ohlcv(df: pd.DataFrame, rule: str) -> pd.DataFrame:
    ...
    try:
        ...
        # We need at least OHLC to be valid.
        subset_cols = ['Open', 'High', 'Low', 'Close']
        resampled.dropna(subset=subset_cols, how='all', inplace=True)
        
        return resampled
    except Exception as e:
        # Fallback for older Pandas versions where 'ME' might not be supported (deprecated since 2.2)
        if rule == 'ME':
            try:
                # Use legacy 'M' rule if 'ME' fails
                resampled = df.resample('M').agg(agg_dict)
...
def align_series(
    ...
    Args:
        ...
        ffill (bool): If True, forward fills NaN values after alignment (useful for outer joins or mismatched holidays).
    ...
    # This ensures that we only compare dates that exist in both (for inner join)
    # or creates NaN where missing (for outer join)
    s1_aligned, s2_aligned = series1.align(series2, join=join)

    if ffill:
        # Forward fill to propagate last known price over holidays/missing days
        s1_aligned = s1_aligned.ffill()
        s2_aligned = s2_aligned.ffill()

    # After ffill, drop any remaining NaNs (e.g., at the start of data if one series starts later)
    # to ensure calculations like pct_change don't fail or return bad data.
    # Note: 'valid_mask' keeps only rows where BOTH are valid.
...
def calculate_relative_strength(
    ...
    """
    # 1. Align data strictly (inner join implies we need both to exist to compute a ratio)
    s_close, b_close = align_series(stock_close, benchmark_close, join='inner')
    
    # We need enough data points for the window
    if len(s_close) < window + 1:
        return pd.Series(dtype=float)

    # 2. Calculate Ratio Line
    # Handle division by zero safely
    b_close_safe = b_close.replace(0, np.nan)
    ratio_line = s_close.div(b_close_safe)
    
    # 3. Calculate Rate of Change of the Ratio
    # This represents "Outperformance" over the window
...
def normalize_ticker(ticker: str) -> str:
    ...
    if t.endswith(".NS") or t.endswith(".BO") or t.endswith(".BSE"):
        return t
