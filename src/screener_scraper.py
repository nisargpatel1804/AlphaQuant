...
class ScreenerScraper:
    ...
    def _ensure_industry_map(self) -> None:
        ...
        if not self._industry_map:
            try:
                ...
                print(f"Warning: Failed to fetch industry master list: {e}")

    def fetch_industry_pe_map(self) -> Dict[str, float]:
        headers = {"User-Agent": random.choice(USER_AGENTS)}
        response = self.session.get(MARKET_INDUSTRIES_URL, headers=headers, timeout=self.timeout)
        response.raise_for_status()
    ...
    @staticmethod
    def _is_consolidated_response(response: requests.Response) -> bool:
        # Check if we were redirected away from consolidated
        return response.status_code == 200 and "/consolidated/" in response.url
...
async def scrape_balance_sheet_with_playwright(ticker: str, target_url: str = None) -> Dict[str, Dict[str, float]]:
    # Use target_url if provided (to respect standalone switch), else default
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Screener.in Data Scraper")
    ...
    if args.ticker and not is_range:
        ...
        logging.info(f"Scraping data for single ticker: {ticker}")
        scraper = ScreenerScraper()
        try:
            data = scraper.fetch_company_payload(ticker)
            data["ticker"] = ticker
            filename = f"json/{ticker}_screener.json"
            with open(filename, "w", encoding="utf-8") as f: json.dump(data, f, indent=4)
            logging.info(f"✅ Successfully scraped and saved to: {filename}")
        except Exception as e:
            logging.error(f"❌ Failed to scrape {ticker}: {e}")
            sys.exit(1)
        sys.exit(0)

    logging.info("Fetching Nifty 500 tickers...")
    try: tickers = get_nifty_tickers()
    except Exception as e:
    ...
    if is_range and start_idx is not None and end_idx is not None:
        ...
        selected_tickers = tickers[start_pos - 1:end_pos]
    ...
    for i, ticker in enumerate(selected_tickers, 1):
        logging.info(f"Processing {i}/{len(selected_tickers)}: {ticker}")
        try:
            data = scraper.fetch_company_payload(ticker)
            data["ticker"] = ticker
            filename = f"json/{ticker}_screener.json"
            ...
            logging.error(f"❌ Failed to scrape {ticker}: {e}")
            failure_count += 1
        if i < len(selected_tickers): time.sleep(1)
